...
class Command(BaseCommand):
    ...
    def replace_line(self, content, old_value, new_value):
        ...
        lines = map(lambda l: l if old_value not in l else f"{new_value}\n", lines)
    ...
    def handle(self, *args, **options):
        ...
        content = self.replace_line(content, "SECRET_KEY", f"SECRET_KEY='{key}'")

        if is_debug:
            content = self.replace_line(content, "DEBUG", "DEBUG=True")
            content = self.replace_line(content, "USE_SQLITE", "USE_SQLITE=True")
            content = self.replace_line(content, "SITE_DOMAIN", "SITE_DOMAIN='*'")
